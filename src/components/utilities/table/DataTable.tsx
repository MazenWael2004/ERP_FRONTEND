...
export const DataTable = <T extends Record<string, unknown>>({
  ...
  //       return (
  //         <div className="flex items-center gap-2">
  //           <Button onClick={() => onEdit?.(row.original)}size={'sm'} variant={'lightprimary'} className="size-8! rounded-full">
  //             <Pencil className="size-5" />
  //           </Button>
  //           <Button onClick={() => onDelete?.(row.original)} size={'sm'} variant={'lighterror'} className="size-8! rounded-full">
  //             <Trash2 className="size-5" />
  //           </Button>
  //         </div>
  //       );
 ...
 const table = useReactTable({
  ...
  const handleDownload = () => {
    ...
    const headers = finalColumns.map((col) => String(col.header));
